...
from olltools import ollobj
from olltools.common.io_encoding import ensure_stdout_handles_unicode
...
def preread_file(args):
    ...
    err_stream, err_dumping, did_dump = \
        ollobj.process_file(filename, data, output_dir=output_dir)
    if did_dump and not err_stream and not err_dumping:
        return ollobj.RETURN_DID_DUMP
    else:
        return ollobj.RETURN_NO_DUMP   # just anything else
...
class TestOleObj(unittest.TestCase):
    ...
    def test_no_output(self):
        ...
        for sample_name in ('sample_with_lnk_to_calc.doc',
            ...
            self.assertEqual(ret_val, ollobj.RETURN_NO_DUMP,
    ...
    def do_test_md5(self, args, test_fun=None, only_run_every=1):
        ...
        for sample_index, (sample_name, embedded_name, expect_hash) \
            ...
            self.assertEqual(ret_val, ollobj.RETURN_DID_DUMP,
                             msg='Wrong return value {} for {}. Output:\n{}'
                                 .format(ret_val, sample_name, output))
...
class TestSaneFilenameCreation(unittest.TestCase):
    ...
    def test_with_empty_inputs(self):
        ...
        iter = ollobj.get_sane_embedded_filenames('', '', '', 10, 47)
        output = set()
    ...
    def test_that_first_has_priority(self):
        iter = ollobj.get_sane_embedded_filenames('fname.sfx', 'do_not.use',
                                                  'do_not.use', 10, 47)
    ...
    def test_that_suffixed_have_priority(self):
        iter = ollobj.get_sane_embedded_filenames('no_suffix', 'also_not',
                                                  'fname.sfx', 10, 47)
        self.assertEqual(next(iter), 'fname.sfx')
    ...
    def test_with_hardly_any_length(self):
        iter = ollobj.get_sane_embedded_filenames('fname.suffx', 'fname.sufx',
                                                  'fname.sfx', 4, 47)
    ...
    def test_with_mean_unicode(self):
        ...
        iter = ollobj.get_sane_embedded_filenames(uni_name1, uni_name2,
                                                  'regular_txt', 30, 47)
    ...
    def test_last_resort(self):
        ...
        self.assertEqual(len(all_options), ollobj.MAX_FILENAME_ATTEMPTS+1)
    ...
    def test_realworld_lnk_example(self):
        ...
        iter = ollobj.get_sane_embedded_filenames(fname, src_path, tmp_path,
                                                  30, 47)
